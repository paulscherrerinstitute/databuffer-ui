...
describe('module util.ts', () => {
	describe('formatDate', () => {
		...
		it('should format a date in the local time zone', () => {
			...
			expect(formatDate(d1.getTime())).toBe('2020-02-03 09:08:07.006')
			expect(formatDate(d2.getTime())).toBe('2020-12-13 19:18:17.016')
			expect(formatDate(d3.getTime())).toBe('2020-12-13 19:18:17.126')
		})
	})
	...
	describe('omitFromArray', () => {
		it('returns empty array on empty array', () => {
			const input: string[] = []
			for (let i = 0; i < 3; i++) {
				expect(omitFromArray(input, i)).toEqual([])
			}
		})

		it('returns complete copy of array on index >= length', () => {
			const input = [10, 20, 30, 40]
			expect(omitFromArray(input, input.length)).not.toBe(input) // not the same array
			expect(omitFromArray(input, input.length)).toEqual(input)
			expect(omitFromArray(input, input.length + 100)).toEqual(input)
		})

		it('works with 0 <= index < length', () => {
			const input = [10, 20, 30, 40]
			expect(omitFromArray(input, 0)).toEqual([20, 30, 40])
			expect(omitFromArray(input, 1)).toEqual([10, 30, 40])
			expect(omitFromArray(input, 2)).toEqual([10, 20, 40])
			expect(omitFromArray(input, 3)).toEqual([10, 20, 30])
		})

		it('works with -length <= index < 0', () => {
			const input = [10, 20, 30, 40]
			expect(omitFromArray(input, -4)).toEqual([20, 30, 40])
			expect(omitFromArray(input, -3)).toEqual([10, 30, 40])
			expect(omitFromArray(input, -2)).toEqual([10, 20, 40])
			expect(omitFromArray(input, -1)).toEqual([10, 20, 30])
		})
	})
